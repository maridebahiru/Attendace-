...
const filePath = 'C:\\Users\\Mar\\Downloads\\_የ4ኛ ሱባዔ ጉባዔ መመዝገቢያ ቅፅ (Responses).xlsx';
...
async function runImport() {
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(async (r, idx) => {
      ...
      const docId = empId; // Use unique employee ID as doc ID to prevent duplicate phone overwrites
    ...
    console.log(`Imported ${Math.min(i + BATCH_SIZE, rows.length)}/${rows.length} records...`);
  }

  console.log("\n--- IMPORT COMPLETE ---");
  console.log(`Total Rows: ${rows.length}`);
  console.log(`Successfully Imported to Firestore: ${inserted}`);
